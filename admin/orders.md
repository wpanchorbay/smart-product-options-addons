...
Smart Product Options and Addons automatically attaches the customer's selected options to every WooCommerce order. No manual setup or custom development is required — the options data is captured, calculated, and saved seamlessly during the checkout process.
...
When viewing any order containing Smart Product Options and Addons selections, the customer's choices are displayed directly below the product name in the **Order Items** admin panel:

![WooCommerce order screen showing an order line item with Smart Product Options and Addons custom options listed beneath the product name](/public/order-line-item.png)
...
Before the order is completed, Smart Product Options and Addons displays selected options on frontend transactional pages:
...
![WooCommerce cart page with Smart Product Options and Addons selections listed under the product](/public/cart-item-options.png)

---

## Pricing on Orders

Smart Product Options and Addons bakes option pricing adjustments directly into the product line item price rather than adding them as separate order fees.
...
By modifying the product line item price directly instead of generating separate "fee lines", Smart Product Options and Addons maintains 100% compatibility with:
...
For options linked to a [Global Stock Item](/stocks/index), Smart Product Options and Addons triggers stock deductions during WooCommerce's stock reduction hook (`woocommerce_reduce_order_stock`):

1. **Checkout Check:** Smart Product Options and Addons does a final live stock check at checkout submit.
2. **Hook Trigger:** Once payment is authorized and WooCommerce reduces catalog stock, Smart Product Options and Addons intercepts the action.
...
If a customer cancels their order or a store admin manually changes the status to **Cancelled** or **Refunded**, Smart Product Options and Addons automatically returns the inventory to the global stock pool:
...
- **Read Stock Intents:** Smart Product Options and Addons reads the `_ob_stock_intents` metadata attached to the order line items to know exactly how much inventory to add back.
- **Duplicate Prevention:** To prevent restoring stock twice if an order status is toggled back and forth, Smart Product Options and Addons writes a `_ob_stock_restored` flag on the order once the restoration completes.

---

## Viewing Order Metadata Directly

For developers, custom integration developers, or troubleshooting purposes, Smart Product Options and Addons stores its raw data on the order line items inside the database:
...
- **`_ob_stock_intents`:** A serialized array stored on the line item meta. It maps the Option IDs to their linked Global Stock Item IDs and the deduction amounts. This is used by Smart Product Options and Addons during status transitions to know how much stock to return on cancellation.
